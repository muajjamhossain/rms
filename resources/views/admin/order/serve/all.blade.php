    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="card-title card_top_title"><i class="fa fa-gg-circle"></i> All Serving Orders Information</h3>
                    </div>
                ...
                <form action="{{ route('typrWiseOrderServe') }}" method="post">
                    @csrf
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <table id="datatable" class="table table-bordered custom_table mb-0 with-image">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @if(Auth::user()->role_id != 2)
                                        <td>
                                            <button class="btn btn-success" id="confirm" onClick="changeStatus('{{ $data->slug }}',4)"><i class="fa fa-check-circle-o" aria-hidden="true"></i> Delivery</button>
                                        </td>
                                        @endif
        ...
        $(document).ready(function() {
            ...
            this.changeStatus = function(slug, action){
                if(action == 4 || action == 5) {
                    $.get('{{ url('admin/serve-order/data/'.$slug) }}/'+slug+'/'+action, function(data){
            ...
            setInterval(function(){ 
                $.get('{{ url('admin/serve-order/loadData/'.$slug) }}', function(data){
                        $('#table').empty().append(data);
                        $('#datatable').DataTable({ "bSort" : false });
                }); 
            }, 10000);
